...
const targetDir = path.resolve(__dirname, "..", "public", "tiles", "pavers", "30x30-square");
...
const gray = await sharp(albedoPath)
  .grayscale()
  .blur(1.35)
  .raw()
  .toBuffer();
...
for (let y = 0; y < height; y += 1) {
  for (let x = 0; x < width; x += 1) {
    const i = y * width + x;
    const v = luma[i];
    // Emphasize ornament grooves while suppressing high-frequency albedo noise.
...
console.log("Generated square maps:", { heightPath, roughnessPath, aoPath, normalPath });
